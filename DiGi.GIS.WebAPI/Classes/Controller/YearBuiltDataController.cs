...
namespace DiGi.GIS.WebAPI.Classes
{
    ...
    public class YearBuiltDataController : DiGi.WebAPI.Classes.WebAPIController
    {
        ...
        public async Task<IActionResult> UpdateItemsByCountyIdsAsync([FromBody] JsonArray? jsonArray, [FromQuery(Name = "countyids")] int[]? countyIds, [FromHeader(Name = "key")] string? key = null, CancellationToken cancellationToken = default)
        {
            ...
            Dictionary<string, int> countyIds_ByReference = await PostgreSQL.Query.CountyIdsByReferencesAsync(building2DPostgreSQLConverter, yearBuiltDatas_GIS.ConvertAll(x => x?.Reference), countyIds_Candidate);
